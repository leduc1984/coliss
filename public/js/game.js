class GameManager {
    ...
    // Legacy method - map editor now opens in new tab via keyboard shortcut
    
    /**
     * Open the integrated map editor (admin only)
     */
    openMapEditor() {
        if (this.user.role !== 'admin' && this.user.role !== 'co-admin') {
        ...
        console.log('🗺️ Opening integrated map editor...');
        
        // Hide game screen and show editor screen
        document.getElementById('game-screen').classList.remove('active');
        document.getElementById('editor-screen').classList.add('active');
        
        // Initialize the integrated editor if not already done
        this.initializeIntegratedEditor();
    }
    
    /**
     * Initialize the integrated map editor
     */
    initializeIntegratedEditor() {
        // Setup close button for integrated editor
        const closeEditorBtn = document.getElementById('closeIntegratedEditor');
        if (closeEditorBtn) {
        ...
        console.log('🎨 Integrated map editor initialized');
    }
    
    /**
     * Close the integrated map editor
     */
    closeMapEditor() {
        console.log('🗺️ Closing integrated map editor...');
        
        // Hide editor screen and show game screen
        document.getElementById('editor-screen').classList.remove('active');
        document.getElementById('game-screen').classList.add('active');
    }
    
    /**
     * Setup editor tools and functionality
     */
    setupEditorTools() {
        ...
        if (editorFileInput) {
            editorFileInput.addEventListener('change', (event) => {
                this.loadMapFile(event.target.files[0]);
            });
        }
        ...
        const placementToolBtn = document.getElementById('placementToolBtn');
        const collisionToolBtn = document.getElementById('collisionToolBtn');
        const pokemonZoneToolBtn = document.getElementById('pokemonZoneToolBtn');
        
        if (placementToolBtn) {
            placementToolBtn.addEventListener('click', () => {
                this.setEditorTool('placement');
            });
        }
        
        if (collisionToolBtn) {
            collisionToolBtn.addEventListener('click', () => {
                this.setEditorTool('collision');
            });
        }
        
        if (pokemonZoneToolBtn) {
            pokemonZoneToolBtn.addEventListener('click', () => {
                this.setEditorTool('pokemonZone');
            });
        }
        
        console.log('🔧 Editor tools initialized');
    }
    
    /**
     * Load a map file for editing
    ...
    async loadMapFile(file) {
        ...
        try {
            ...
            // Clear existing editor meshes
            if (this.loadedEditorMeshes) {
                this.loadedEditorMeshes.forEach(mesh => mesh.dispose());
            }
            this.loadedEditorMeshes = [];
            
            // Load the new map with GLB plugin
            const result = await BABYLON.SceneLoader.ImportMeshAsync("", "", url, this.scene, null, ".glb");
            
            if (result.meshes.length > 0) {
                this.loadedEditorMeshes = result.meshes;
                console.log(`✅ Map loaded: ${result.meshes.length} meshes`);
                
                // Update status
                const statusText = document.getElementById('editorStatusText');
                if (statusText) {
                    statusText.textContent = `Loaded: ${file.name} (${result.meshes.length} meshes)`;
                }
            }
            ...
            const statusText = document.getElementById('editorStatusText');
            if (statusText) {
                statusText.textContent = 'Error loading map file';
            }
        }
    }
    ...
    setEditorTool(toolName) {
        ...
        const toolBtn = document.getElementById(`${toolName}ToolBtn`);
        if (toolBtn) {
        ...
        console.log(`🔨 Editor tool changed to: ${toolName}`);
    }
